...
class LearningGoalController {
  ...
  @PutMapping("/{id}")
  LearningGoalResponse change(
      @PathVariable UUID id, @Valid @RequestBody LearningGoalRequest request) {
    return LearningGoalResponse.from(
        service.changeGoal(
            id,
            new ChangeLearningGoalCommand(
                request.title(),
                request.description(),
                request.startDate(),
                request.targetDate())));
