...
class CertificationController {
  ...
  CertificationResponse register(@Valid @RequestBody RegisterCertificationRequest request) {
    return CertificationResponse.from(
        service.registerCertification(
            new RegisterCertificationCommand(
                request.qualificationName(), request.relatedLearningGoalId())));
  }
  ...
  List<CertificationResponse> upcoming() {
    return service.findUpcomingExams(LocalDate.now()).stream()
        .map(CertificationResponse::from)
        .toList();
  }

  @PostMapping("/{id}/exam-plan")
  CertificationResponse schedule(
      @PathVariable UUID id, @Valid @RequestBody ScheduleExamRequest request) {
    return CertificationResponse.from(
        service.scheduleExam(id, new ScheduleExamCommand(request.plannedExamDate())));
  }

  @PostMapping("/{id}/exam-results")
  CertificationResponse recordResult(
      @PathVariable UUID id, @Valid @RequestBody RecordExamResultRequest request) {
    return CertificationResponse.from(
        service.recordExamResult(
            id,
            new RecordExamResultCommand(
                request.examDate(), request.resultDate(), request.outcome(), request.note())));
  }

  record RegisterCertificationRequest(
      @NotBlank String qualificationName, UUID relatedLearningGoalId) {}
  ...
  record ExamAttemptResponse(
      UUID id, LocalDate examDate, LocalDate resultDate, String outcome, String note) {
