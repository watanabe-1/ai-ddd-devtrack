...
interface SpringDataStudySessionRepository extends JpaRepository<StudySessionJpaEntity, UUID> {
    ...
    @Query("select coalesce(sum(s.durationMinutes), 0) from StudySessionJpaEntity s where s.learningGoalId = :learningGoalId")
    long sumDurationByLearningGoalId(@Param("learningGoalId") UUID learningGoalId);

    @Query("select coalesce(sum(s.durationMinutes), 0) from StudySessionJpaEntity s where s.studyDate between :startDate and :endDate")
    long sumDurationBetween(@Param("startDate") LocalDate startDate, @Param("endDate") LocalDate endDate);
}
